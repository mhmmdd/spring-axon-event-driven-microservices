...
public class EventsReplayController {
    ...
    public ResponseEntity<String> replayEvents(@PathVariable String processorName) {
        ...
        if (trackingEventProcessor.isPresent()) {
            ...
            return ResponseEntity.badRequest().body(String.format("The event processor with a name [%s] is not " +
                            "a tracking event processor. Only Tracking event processor is supported", processorName));
        }
    }
}
